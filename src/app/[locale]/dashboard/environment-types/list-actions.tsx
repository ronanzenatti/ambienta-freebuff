...
import { deleteEnvironmentType, toggleEnvironmentTypeActive } from "@/actions/environment-types";
...
export function ListActions({ id, name, active }: { id: string; name: string; active: boolean }) {
  ...
  const handleToggle = async () => {
    await toggleEnvironmentTypeActive(id);
  };

  const handleDelete = async () => {
    setDeleting(true);
    const result = await deleteEnvironmentType(id);
    setDeleting(false);
  ...
  return (
    <>
      <div className="flex items-center justify-end gap-1">
        <Link
          href={`/dashboard/environment-types/${id}/edit`}
      ...
      <ConfirmModal
        ...
        title="Excluir Tipo de Ambiente"
