...
import type { ActionState } from "@/actions/environment-types";
...
export function EditForm({ item, updateAction }: Props) {
  const [state, action, pending] = useActionState<ActionState, FormData>(updateAction, {});

  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/dashboard/environment-types" className="p-2 rounded-lg text-text-muted hover:text-text hover:bg-surface-hovered transition-colors">
      ...
      <div className="bg-white rounded-xl border border-border p-6">
        ...
        <form action={action} className="space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Link href="/dashboard/environment-types" className="px-4 py-2.5 text-sm font-medium text-text-subtle hover:text-text bg-surface hover:bg-surface-hovered border border-border rounded-xl transition-colors">Cancelar</Link>
          </div>
        </form>
      </div>
    </div>
