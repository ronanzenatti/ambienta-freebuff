...
async function Table({ q, page }: { q: string; page: number }) {
  const { data, total, page: currentPage, totalPages } = await listEnvironmentTypes({ q, page });

  if (data.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-border p-12 text-center">
        <Tags className="w-12 h-12 text-text-muted mx-auto mb-4" />
        <h3 className="text-lg font-medium text-text mb-1">Nenhum tipo de ambiente</h3>
        <p className="text-sm text-text-subtle mb-4">
          {q ? "Nenhum resultado para sua busca." : "Crie o primeiro tipo de ambiente para começar."}
        </p>
        {!q && (
          <Link
            href="/dashboard/environment-types/new"
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-surface-subtle">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-text-muted uppercase tracking-wider">Descrição</th>
            ...
            {data.map((item) => (
              <tr key={item.id} className="hover:bg-surface-hovered transition-colors">
                ...
                <td className="px-4 py-3 text-right">
                  <ListActions id={item.id} name={item.name} active={item.active} />
                </td>
              </tr>
            ))}
          </tbody>
...
export default async function EnvironmentTypesPage({ searchParams }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-text">Tipos de Ambiente</h1>
          <p className="mt-1 text-sm text-text-subtle">Gerencie os tipos de ambientes disponíveis no sistema.</p>
        </div>
        <Link
          href="/dashboard/environment-types/new"
