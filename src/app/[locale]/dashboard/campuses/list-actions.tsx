...
import { deleteCampus, toggleCampusActive } from "@/actions/campuses";

export function CampusListActions({ id, name, active }: { id: string; name: string; active: boolean }) {
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [deleting, setDeleting] = useState(false);

  const handleToggle = async () => { await toggleCampusActive(id); };
  ...
  return (
    <>
      <div className="flex items-center justify-end gap-1">
        <Link href={`/dashboard/campuses/${id}/edit`} className="p-2 rounded-lg text-text-muted hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Editar">
