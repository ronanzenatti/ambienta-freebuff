...
import { deleteBuilding, toggleBuildingActive } from "@/actions/buildings";

export function BuildingListActions({ id, name, active }: { id: string; name: string; active: boolean }) {
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [deleting, setDeleting] = useState(false);

  const handleToggle = async () => { await toggleBuildingActive(id); };
  ...
  return (
    <>
      <div className="flex items-center justify-end gap-1">
        <Link href={`/dashboard/buildings/${id}/edit`} className="p-2 rounded-lg text-text-muted hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Editar">
